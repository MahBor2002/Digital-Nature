class Bunny {
  constructor() {
    ...
    this.maxForce = 0.3;
    this.maxSpeed = 1;
    this.col = color(255, 10, 100); //color(random(0, 255), random(0, 255), random(0, 255));
    this.eatenRadius = 15;
  ...
  show() {
    ...
    stroke(255, 0, 0, 20);
    circle(this.position.x, this.position.y, this.eatenRadius * 2);
    stroke(100, 200, 0, 50);
    circle(this.position.x, this.position.y, this.seeingFood * 2);
    
    stroke(255, 0, 0, 50);
    circle(this.position.x, this.position.y, this.fleeRadius * 2);
  ...
  seek(findTarget) {
    ...
    for (let i = 0; i < findTarget.length; i++) {
      ...
      if (d < this.seeingFood && d < closest) {
        closest = d;
  ...
  flee(enemies) {
    ...
    for (let i = 0; i < enemies.length; i++) {
     d = p5.Vector.dist(this.position, enemies[i].position); 
    }
    if (d < this.fleeRadius) {
      return this.seek(enemies).mult(-1);
    }
  }
  
  getEatenByWolves(eater) {
